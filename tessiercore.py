...
class Plot3DSlices:
	...
	def __init__(
		self, data, n_index=None, meshgrid=False, hilbert=False,
		fiddle=True, uniques_col_str=[], style=[], clim=None,
		aspect='auto', interpolation='none'):
		'''
		Constructor. Parameters:
			data: the measured data to plot (already read from file)
			n_index: which plot to plot (for which value of stepval
				columns I think), None for all
		...
		self.filterdata = self.data.sort(self.cols[:-1]).dropna(how='any')
		...
		self.sweepdirection = \
			self.data[self.cols[-1]][0] > self.data[self.cols[-1]][1]

		self.uniques_col_str = list(uniques_col_str)
		for i in self.uniques_col_str:
			col = getattr(self.filterdata, i)
		...
		if n_index is None:
			self.nplots = int(np.product(self.uniques_per_col))
		else:
		...
		#self.fig = plt.figure()
		(self.fig, self.ax) = plt.subplots(self.nplots, 1)
		if self.nplots == 1:
			self.ax = [self.ax]
		self.fig.subplots_adjust(
			top=0.96, bottom=0.03, left=0.1, right=0.9, hspace=0.0)

		cnt = 0
		# enumerate over the generated list of unique values
		# specified in the uniques columns
		for j, ind in enumerate(buildLogicals(self.uniques_col)):
			if n_index is None or j in n_index:
				self.create_subplot(
					cnt, ind, hilbert, style, clim, aspect, interpolation)
				cnt += 1
	...
	def create_subplot(
		...
		# get all the last columns, that we assume contain the to be plotted data
		x = slicy.iloc[:, -3]
		y = slicy.iloc[:, -2]
		z = slicy.iloc[:, -1]
		...
		# if the measurement is not complete this will probably fail
		# so trim of the final sweep?
		print('xu: {:d}, yu: {:d}, len(z): {:d}'.format(xu, yu, len(z)))

		if xu * yu != len(z):
			xu = int(len(z) / yu)  # Floor value
			print('xu: {:d}, yu: {:d}, len(z): {:d}'.format(xu, yu, len(z)))

		# trim the first part of the sweep, for different min max,
		# better to trim last part?
		...
		# determine stepsize for di/dv, inprincipe only y step is used
		# (ie. the diff is also taken in this direction and the measurement swept..)
...
#                 Z = griddata(x,y,Z,xi,yi)
		...
		#ax = self.fig.add_subplot(self.nplots, 1, cnt + 1)
		ax = self.ax[cnt]
		cbar_title = ''
		...
		cbar_title = '{:s}{:s} [{:s}]'.format(
			cbar_trans_formatted, w.cbar_quantity, w.cbar_unit)
		...
		title = '\n'.join(
			['{:s}: {:g} (mV)'.format(i, getattr(slicy, i).iloc[0])
				for i in self.uniques_col_str])
		print(title)
		if w.has_title:
			ax.set_title(title)
	...
	def add_fiddle(self):
		...
		axFiddle = self.fig.add_axes([0.1, 0.85, 0.15, 0.075])
